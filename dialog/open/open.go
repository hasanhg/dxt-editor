...
var _ dialog.Dialog = (*OpenDialog)(nil)

func (f *openDialog) makeUI() fyne.CanvasObject {
...
func (f *openDialog) optionsMenu(position fyne.Position, buttonSize fyne.Size) {
...
func (f *openDialog) loadFavorites() {
...
func (f *openDialog) refreshDir(dir fyne.ListableURI) {
	f.files.Objects = nil
	...
	if parent != nil && parent.String() != dir.String() {
		fi := &openDialogItem{picker: f, name: "(Parent)", location: parent, dir: true}
...
func (f *openDialog) setLocation(dir fyne.URI) error {
	if dir == nil {
...
func (f *openDialog) setSelected(file *openDialogItem) {
	if f.selected != nil {
...
func (f *openDialog) setView(view viewLayout) {
	f.view = view
	if f.view == gridView {
...
//
func (f *OpenDialog) effectiveStartingDir() fyne.ListableURI {
...
func showFile(file *OpenDialog) *openDialog {
	d := &openDialog{file: file, initialFileName: file.initialFileName}
	ui := d.makeUI()
...
//
// Since: 2.1
func (f *OpenDialog) MinSize() fyne.Size {
	return f.dialog.win.MinSize()
}

// Show shows the file dialog.
func (f *OpenDialog) Show() {
...
func (f *OpenDialog) Resize(size fyne.Size) {
	f.desiredSize = size
	if f.dialog == nil {
		return
	}
...
func (f *OpenDialog) SetDismissText(label string) {
	if f.dialog == nil {
		return
	}
...
//
// Since: 1.4
func (f *OpenDialog) SetLocation(u fyne.ListableURI) {
...
func (f *OpenDialog) SetOnClosed(closed func()) {
	if f.dialog == nil {
		return
	}
...
func (f *OpenDialog) SetFilter(filter storage.FileFilter) {
	if f.isDirectory() {

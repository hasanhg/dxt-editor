...
type selectFolderDialog struct {
	...
	win      *widget.PopUp
	selected *selectFolderDialogItem
	dir      fyne.ListableURI
	// this will be the initial filename in a OpenDialog in save mode
	initialFileName string
}

// OpenDialog is a dialog containing a file picker for use in opening or saving files.
type SelectFolderDialog struct {
	callback         interface{}
	onClosedCallback func(bool)
	filter           storage.FileFilter
	parent           fyne.Window
	dialog           *selectFolderDialog
	dismissText      string
...
var _ dialog.Dialog = (*SelectFolderDialog)(nil)

func (f *selectFolderDialog) makeUI() fyne.CanvasObject {
...
func (f *selectFolderDialog) optionsMenu(position fyne.Position, buttonSize fyne.Size) {
...
func (f *selectFolderDialog) loadFavorites() {
...
func (f *selectFolderDialog) refreshDir(dir fyne.ListableURI) {
	f.files.Objects = nil
	...
	if parent != nil && parent.String() != dir.String() {
		fi := &selectFolderDialogItem{picker: f, name: "(Parent)", location: parent, dir: true}
...
func (f *selectFolderDialog) setLocation(dir fyne.URI) error {
	if dir == nil {
...
func (f *selectFolderDialog) setSelected(file *selectFolderDialogItem) {
	if f.selected != nil {
...
func (f *selectFolderDialog) setView(view viewLayout) {
	f.view = view
	if f.view == gridView {
...
//
func (f *SelectFolderDialog) effectiveStartingDir() fyne.ListableURI {
...
func showFile(file *SelectFolderDialog) *selectFolderDialog {
	d := &selectFolderDialog{file: file, initialFileName: file.initialFileName}
	ui := d.makeUI()
...
//
// Since: 2.1
func (f *SelectFolderDialog) MinSize() fyne.Size {
	return f.dialog.win.MinSize()
}

// Show shows the file dialog.
func (f *SelectFolderDialog) Show() {
...
func (f *SelectFolderDialog) Refresh() {
...
func (f *SelectFolderDialog) Resize(size fyne.Size) {
	f.desiredSize = size
	if f.dialog == nil {
		return
	}
...
func (f *SelectFolderDialog) Hide() {
	if f.dialog == nil {
		return
	}
	f.dialog.win.Hide()
...
func (f *SelectFolderDialog) SetDismissText(label string) {
	if f.dialog == nil {
		return
	}
...
//
// Since: 1.4
func (f *SelectFolderDialog) SetLocation(u fyne.ListableURI) {
...
func (f *SelectFolderDialog) SetOnClosed(closed func()) {
	if f.dialog == nil {
		return
	}
...
func (f *SelectFolderDialog) SetFilter(filter storage.FileFilter) {
	if f.isDirectory() {
...
func NewFileOpen(fileType string, callback func(fyne.URIReadCloser, error), parent fyne.Window) *SelectFolderDialog {
	dialog := &SelectFolderDialog{callback: callback, parent: parent, fileType: fileType}
	return dialog
}

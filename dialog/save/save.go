...
type saveDialog struct {
	...
	win      *widget.PopUp
	selected *saveDialogItem
	dir      fyne.ListableURI
	// this will be the initial filename in a SaveDialog in save mode
	initialFileName string
}

// SaveDialog is a dialog containing a file picker for use in opening or saving files.
type SaveDialog struct {
	callback         interface{}
...
var _ dialog.Dialog = (*SaveDialog)(nil)

func (f *saveDialog) makeUI() fyne.CanvasObject {
	...
	footer := container.NewBorder(nil, nil, nil, nil,
		container.NewGridWithRows(2,
			f.fileName,
			container.NewHBox(f.fileName, buttons),
		),
	)
...
func (f *saveDialog) optionsMenu(position fyne.Position, buttonSize fyne.Size) {
...
func (f *saveDialog) loadFavorites() {
...
func (f *saveDialog) refreshDir(dir fyne.ListableURI) {
	f.files.Objects = nil
	...
	if parent != nil && parent.String() != dir.String() {
		fi := &saveDialogItem{picker: f, name: "(Parent)", location: parent, dir: true}
...
func (f *saveDialog) setLocation(dir fyne.URI) error {
	if dir == nil {
...
func (f *saveDialog) setSelected(file *saveDialogItem) {
	if f.selected != nil {
...
func (f *saveDialog) setView(view viewLayout) {
	f.view = view
	if f.view == gridView {
...
//
func (f *SaveDialog) effectiveStartingDir() fyne.ListableURI {
...
func showFile(file *SaveDialog) *saveDialog {
	d := &saveDialog{file: file, initialFileName: file.initialFileName}
	ui := d.makeUI()
...
//
// Since: 2.1
func (f *SaveDialog) MinSize() fyne.Size {
	return f.dialog.win.MinSize()
}

// Show shows the file dialog.
func (f *SaveDialog) Show() {
...
func (f *SaveDialog) Resize(size fyne.Size) {
	f.desiredSize = size
	if f.dialog == nil {
		return
	}
...
func (f *SaveDialog) SetDismissText(label string) {
	if f.dialog == nil {
		return
	}
...
//
// Since: 1.4
func (f *SaveDialog) SetLocation(u fyne.ListableURI) {
...
func (f *SaveDialog) SetOnClosed(closed func()) {
	if f.dialog == nil {
		return
	}
...
func (f *SaveDialog) SetFilter(filter storage.FileFilter) {
	if f.isDirectory() {
...
// SetFileName sets the filename in a SaveDialog in save mode.
// This is normally called before the dialog is shown.
func (f *SaveDialog) SetFileName(fileName string) {
	f.initialFileName = fileName
	//Update entry if saveDialog has already been created
	if f.dialog != nil {
...
func NewFileSave(callback func(fyne.URIWriteCloser, error), parent fyne.Window) *SaveDialog {
	dialog := &SaveDialog{callback: callback, parent: parent}
	return dialog
}
